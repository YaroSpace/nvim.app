...
(defn compact-table-header []
  [:div {:class "flex items-center py-2 px-2 border-b-2 text-xs font-semibold text-green-800"
   ...
   [:div {:class "w-24 sm:w-48 flex-shrink-0"} "Plugin"]
   [:div {:class "min-w-16 sm:w-30 flex-shrink-0 text-center justify-evenly"} "Stars"]
   ...
   [:div {:class "min-w-16 sm:w-30 flex-shrink-0"}
    (when (pos? stars)
      [:div {:class "flex items-center space-x-1 text-yellow-600"}
       (star stars star-icon "text-xs text-yellow-500" "Total")
       (star (- stars stars_month) growth-icon-m "text-xs text-red-500 hidden sm:flex" "Stars since beginning of the month")])]
